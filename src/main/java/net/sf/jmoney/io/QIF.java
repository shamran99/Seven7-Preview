...
public class QIF implements FileFormat {
	...
	public void importFile(Session session, File qifFile) {
		try {
			...
			if (header.startsWith("!Type:Bank") || header.startsWith("!Type:Cash") || header.startsWith("!Type:Bar") || // MS
																														// M*ney97
																														// german
																														// edition
					header.startsWith("!Type:CCard") || header.startsWith("!Type:Oth A")
	 ...
	 */
	private void importAccount(Session session, Account account, BufferedReader buffer)
	...
	private SplittedEntry extractSplittedCategory(Session session, Accounts accounts, Account account, Entry entry,
			String line) throws CanceledException {
	...
	private Entry extractCategory(Session session, Accounts accounts, Account account, Entry entry, String line)
	...
	private void removeSimilarTransfer(Account a1, Entry newEntry) {
		if (newEntry instanceof SplittedEntry) {
			...
			for (int i = 0; i < se.getEntries().size(); i++) {
				...
				if (subEntry instanceof DoubleEntry) {
					...
					for (int j = 0; j < a2.getEntries().size(); j++) {
						...
						if ((newDe2 != oldE2) && (oldE2 instanceof DoubleEntry)
								&& (newDe2.getAmount() == oldE2.getAmount())
								&& equals(newDe2.getDate(), oldE2.getDate())
								&& equals(newDe2.getCategory(), oldE2.getCategory())) {
			...
			for (int i = 0; i < a1.getEntries().size(); i++) {
				...
				if ((newDe != e) && (e instanceof DoubleEntry) && equals(newDe.getCategory(), e.getCategory())
						&& (newDe.getAmount() == e.getAmount()) && equals(newDe.getDate(), e.getDate())
						&& equals(newDe.getCheck(), e.getCheck()) && equals(newDe.getDescription(), e.getDescription())
	...
	/**
	 * Auxiliary class with the function of a table with the columns "names" and
	 * "accounts". Provides a method "getAccount()" which returns the
	 * corresponding account to a given account name. If there is no entry in
	 * the table the user will be asked to choose an account.
	 */
	class Accounts {
