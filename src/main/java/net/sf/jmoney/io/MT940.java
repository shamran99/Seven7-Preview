...
public class MT940 implements FileFormat {
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.jmoney.FileFormat#importFile(net.sf.jmoney.model.Session,
	 * java.io.File)
	 */
	...
	private boolean importTransaction(Session session, BufferedReader reader) throws IOException {
		...
		while (!ready) {
			...
			if (field.equals("25")) {
				...
				// Next: (C)redit, (R)eversal (C)redit, (D)ebit or (R)eversal
				// (D)ebit
				String creditDebit;
				...
				/*
				 * // setCreation(long aCreation) { setCheck(String aCheck) {
				 * setCategory(Category aCategory) { // setStatus(int aStatus) {
				 */
	 ...
	 * (non-Javadoc)
	 * 
	 * @see net.sf.jmoney.FileFormat#exportAccount(net.sf.jmoney.model.Session,
	 * net.sf.jmoney.model.Account, java.io.File)
	 */
