...
public class Entry implements Serializable {
	...
	protected void copyValues(Entry newEntry) {
		...
		itemCode = newEntry.getItemCode();
		quantity = newEntry.getQuantity();
		cost = newEntry.getCost();
	}
	...
	public String getStatusString() {
		...
		text[RECONCILING] = Constants.LANGUAGE.getString("Entry.reconcilingShort");
	...
	public void setItemCode(String anItemCode) {
		if (itemCode != null && itemCode.equals(anItemCode))
			return;
		itemCode = anItemCode == null || anItemCode.length() == 0 ? null : anItemCode;
		changeSupport.firePropertyChange("itemCode", null, itemCode);
	}
	...
	public void setQuantity(int aQuantity) {
		if (aQuantity == quantity)
			return;
		quantity = aQuantity;
		changeSupport.firePropertyChange("quantity", null, aQuantity);
	}
	...
	public void setCost(float aCost) {
		if (aCost == cost)
			return;
		cost = aCost;
		changeSupport.firePropertyChange("cost", null, aCost);
	}
