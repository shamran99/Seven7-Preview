...
public class SplittedEntry extends Entry {
	...
	public void setEntryAt(Entry newEntry, int index) {
		...
		if (oldEntry instanceof DoubleEntry)
			((DoubleEntry) oldEntry).removeOther();
		entries.setElementAt(newEntry, index);
		if (newEntry instanceof DoubleEntry)
			((DoubleEntry) newEntry).addOther();
	}
